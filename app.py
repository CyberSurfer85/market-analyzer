...
tabs = st.tabs(["⭐ Watchlist", "📋 Scanner", "📈 Grafico & Dettaglio", "🏛️ Fondamentali", "🌐 Macro", "📝 Note", "ℹ️ Guida"])
...
# ────────────────────────────────────────────────────
with tabs[4]:
    st.subheader("🌐 Contesto Macro — USA & Globale")
    st.caption("Fonte: Federal Reserve di St. Louis (FRED) · Aggiornamento ogni ora · Dati gratuiti e ufficiali")
    st.markdown("---")

    # ── CURVA DEI RENDIMENTI ──────────────────────────────
    st.markdown("### 📈 Curva dei Rendimenti USA")
    st.caption(
        "Confronto tra tassi a breve (2 anni) e lungo termine (10 anni). "
        "Spread negativo = curva invertita = segnale storico di rallentamento economico."
    )

    with st.spinner("Caricamento dati obbligazionari..."):
        t2y  = fetch_fred("DGS2",  120)   # Treasury 2 anni
        t10y = fetch_fred("DGS10", 120)   # Treasury 10 anni
    ...
    if not t2y.empty and not t10y.empty:
        ...
        combined["Spread (10Y-2Y)"] = combined["10 anni"] - combined["2 anni"]

        # Valori attuali
        curr_2y     = combined["2 anni"].iloc[-1]
        curr_10y    = combined["10 anni"].iloc[-1]
        curr_spread = combined["Spread (10Y-2Y)"].iloc[-1]

        k1, k2, k3 = st.columns(3)
        k1.metric("Treasury 2 anni",  f"{curr_2y:.2f}%")
        k2.metric("Treasury 10 anni", f"{curr_10y:.2f}%")
        spread_color = "🔴 Invertita" if curr_spread < 0 else "🟢 Normale"
        k3.metric("Spread 10Y-2Y", f"{curr_spread:+.2f}%", delta=spread_color)

        fig_curve = go.Figure()
        fig_curve.add_trace(go.Scatter(
            x=combined.index, y=combined["2 anni"],
            name="Treasury 2 anni", line=dict(color="#e74c3c", width=2)
        ))
        fig_curve.add_trace(go.Scatter(
            x=combined.index, y=combined["10 anni"],
            name="Treasury 10 anni", line=dict(color="#2ecc71", width=2)
        ))
        fig_curve.add_hline(y=0, line_dash="dot", line_color="gray")
        fig_curve.add_trace(go.Scatter(
            x=combined.index, y=combined["Spread (10Y-2Y)"],
            name="Spread 10Y-2Y", line=dict(color="#f39c12", width=1.5, dash="dash"),
            fill="tozeroy",
        ...
        fig_curve.update_yaxes(gridcolor="rgba(255,255,255,0.06)")
        st.plotly_chart(fig_curve, use_container_width=True)

        if curr_spread < 0:
            st.warning(
                f"⚠️ La curva è **invertita** ({curr_spread:+.2f}%). "
                "Storicamente questo ha preceduto ogni recessione USA con 12-18 mesi di anticipo. "
                "Non è un segnale di vendita immediata, ma suggerisce cautela su asset ciclici."
            )
        else:
            st.success(
                f"✅ La curva è **normale** ({curr_spread:+.2f}%). "
                "Il mercato obbligazionario non segnala stress recessivo immediato."
            )
    else:
        st.error("Impossibile caricare dati FRED. Verifica la connessione internet.")

    st.markdown("---")

    # ── INFLAZIONE CPI ────────────────────────────────────
    st.markdown("### 💰 Inflazione USA (CPI YoY %)")
    st.caption(
        "Variazione annua dei prezzi al consumo. Target Fed: 2%. "
        "La direzione conta più del numero assoluto."
    )

    with st.spinner("Caricamento dati inflazione..."):
        cpi = fetch_fred("CPIAUCSL", 36)   # CPI mensile

    if not cpi.empty:
        cpi["YoY"] = cpi["value"].pct_change(12) * 100
        cpi_clean = cpi.dropna()

        curr_cpi  = cpi_clean["YoY"].iloc[-1]
        prev_cpi  = cpi_clean["YoY"].iloc[-2]
        delta_cpi = curr_cpi - prev_cpi

        k1, k2, k3 = st.columns(3)
        k1.metric("CPI attuale (YoY)", f"{curr_cpi:.1f}%")
        k2.metric("Mese precedente",   f"{prev_cpi:.1f}%")
        direzione = "↓ In calo" if delta_cpi < 0 else "↑ In salita"
        d_color   = "🟢" if delta_cpi < 0 and curr_cpi > 2 else "🔴" if delta_cpi > 0 else "🟡"
        k3.metric("Direzione", f"{d_color} {direzione}")

        fig_cpi = go.Figure()
        fig_cpi.add_trace(go.Scatter(
            x=cpi_clean.index, y=cpi_clean["YoY"],
            name="CPI YoY %", line=dict(color="#9b59b6", width=2),
            fill="tozeroy", fillcolor="rgba(155,89,182,0.08)"
        ))
        fig_cpi.add_hline(y=2, line_dash="dot", line_color="#2ecc71",
                          annotation_text="Target Fed 2%")
        fig_cpi.update_layout(
            height=260, template="plotly_dark",
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            margin=dict(l=0, r=0, t=20, b=0),
            showlegend=False, yaxis_title="CPI %"
        )
        fig_cpi.update_yaxes(gridcolor="rgba(255,255,255,0.06)")
        st.plotly_chart(fig_cpi, use_container_width=True)

        if curr_cpi > 3 and delta_cpi > 0:
            st.warning(
                f"⚠️ Inflazione al {curr_cpi:.1f}% e **in salita**. "
                "La Fed potrebbe mantenere i tassi alti più a lungo. "
                "Contesto meno favorevole per asset growth e obbligazioni a lungo termine."
            )
        elif curr_cpi <= 2.5 or delta_cpi < 0:
            st.success(
                f"✅ Inflazione al {curr_cpi:.1f}% e **in calo**. "
                "Avvicinamento al target Fed. Possibile apertura a tagli dei tassi."
            )
        else:
            st.info(
                f"🟡 Inflazione al {curr_cpi:.1f}%. "
                "Nella fascia di attenzione — monitorare la direzione nei prossimi mesi."
            )
    else:
        st.error("Impossibile caricare dati CPI da FRED.")

    st.markdown("---")

    # ── DOLLARO DXY ───────────────────────────────────────
    st.markdown("### 💵 Forza del Dollaro (DXY)")
    st.caption(
        "Indice del dollaro USA vs paniere di valute principali. "
        "Dollaro forte = pressione su materie prime e multinazionali USA. "
        "Dollaro debole = favorevole per mercati emergenti e commodity."
    )

    with st.spinner("Caricamento DXY..."):
        dxy_df = fetch_ticker("DX-Y.NYB", "1y", "1d")

    if not dxy_df.empty:
        dxy_close = dxy_df["Close"]
        curr_dxy  = dxy_close.iloc[-1]
        prev_dxy  = dxy_close.iloc[-2]
        chg_dxy   = (curr_dxy - prev_dxy) / prev_dxy * 100
        ma50_dxy  = dxy_close.rolling(50).mean().iloc[-1]
        ma200_dxy = dxy_close.rolling(200).mean().iloc[-1]

        k1, k2, k3 = st.columns(3)
        k1.metric("DXY attuale", f"{curr_dxy:.2f}")
        k2.metric("Var. giorno", f"{chg_dxy:+.2f}%")
        trend_dxy = "↑ Forte" if curr_dxy > ma50_dxy > ma200_dxy else "↓ Debole" if curr_dxy < ma50_dxy < ma200_dxy else "→ Laterale"
        k3.metric("Trend", trend_dxy)

        fig_dxy = go.Figure()
        fig_dxy.add_trace(go.Scatter(
            x=dxy_df.index, y=dxy_close,
            name="DXY", line=dict(color="#4a90d9", width=2)
        ))
        if len(dxy_df) >= 50:
            fig_dxy.add_trace(go.Scatter(
                x=dxy_df.index, y=dxy_close.rolling(50).mean(),
                name="MA 50", line=dict(color="#f39c12", width=1.5, dash="dot")
            ))
        if len(dxy_df) >= 200:
            fig_dxy.add_trace(go.Scatter(
                x=dxy_df.index, y=dxy_close.rolling(200).mean(),
                name="MA 200", line=dict(color="#e74c3c", width=1.5, dash="dash")
            ))
        ...
        fig_dxy.update_yaxes(gridcolor="rgba(255,255,255,0.06)")
        st.plotly_chart(fig_dxy, use_container_width=True)

        if "Forte" in trend_dxy:
            st.warning(
                "⚠️ Dollaro in trend **rialzista**. "
                "Pressione sulle materie prime e sui ricavi esteri delle multinazionali USA. "
                "Mercati emergenti sotto stress da debito in dollari."
            )
        elif "Debole" in trend_dxy:
            st.success(
                "✅ Dollaro in trend **ribassista**. "
                "Favorevole per materie prime, mercati emergenti e multinazionali USA con ricavi esteri."
            )
        else:
            st.info("🟡 Dollaro laterale. Nessun segnale direzionale chiaro.")
    else:
        st.error("Impossibile caricare dati DXY.")

    st.markdown("---")

    # ── LETTURA INTEGRATA ─────────────────────────────────
    st.markdown("### 🔍 Lettura integrata dei tre indicatori")
    st.caption("Aggiornata automaticamente in base ai dati caricati.")

    if not t2y.empty and not cpi.empty and not dxy_df.empty:
        segnali = []
        if curr_spread < 0:
            segnali.append(("🔴", "Curva invertita", "Segnale storico di rallentamento"))
        elif curr_spread > 0.5:
            segnali.append(("🟢", "Curva normale", "Nessun segnale recessivo"))
        else:
            segnali.append(("🟡", "Curva piatta", "Zona di attenzione"))

        if curr_cpi <= 2.5 or delta_cpi < 0:
            segnali.append(("🟢", f"Inflazione {curr_cpi:.1f}%", "In direzione target Fed"))
        elif curr_cpi > 3 and delta_cpi > 0:
            segnali.append(("🔴", f"Inflazione {curr_cpi:.1f}%", "Sopra target e in salita"))
        else:
            segnali.append(("🟡", f"Inflazione {curr_cpi:.1f}%", "Da monitorare"))

        if "Forte" in trend_dxy:
            segnali.append(("🔴", "Dollaro forte", "Pressione su commodity e EM"))
        elif "Debole" in trend_dxy:
            segnali.append(("🟢", "Dollaro debole", "Favorevole per risk assets"))
        else:
            segnali.append(("🟡", "Dollaro laterale", "Nessun segnale direzionale"))

        verdi  = sum(1 for s in segnali if s[0] == "🟢")
        rossi  = sum(1 for s in segnali if s[0] == "🔴")

        for icona, titolo, desc in segnali:
            st.markdown(f"{icona} **{titolo}** — {desc}")

        st.markdown("")
        if verdi == 3:
            st.success("✅ Contesto macro complessivamente **favorevole** per asset rischiosi.")
        elif rossi == 3:
            st.error("🔴 Contesto macro complessivamente **sfavorevole**. Privilegiare difensivi e liquidità.")
        elif rossi >= 2:
            st.warning("⚠️ Contesto macro **misto con prevalenza negativa**. Cautela sulle nuove posizioni.")
        else:
            st.info("🟡 Contesto macro **misto**. Procedere con analisi titolo per titolo.")

    st.caption(
        "⚠️ La lettura macro è un contesto, non un segnale operativo. "
        "Consulta questa sezione una volta a settimana, non quotidianamente."
    )
